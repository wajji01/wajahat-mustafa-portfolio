...
const socials = [
  { icon: "GH", label: "GitHub",   handle: "@wajahat-dev",    href: "https://github.com"   },
  { icon: "LI", label: "LinkedIn", handle: "Wajahat Mustafa", href: "https://linkedin.com" },
...
export default function Contact() {
  ...
  const inputStyle = (field: string): React.CSSProperties => ({
    ...
    padding: "0.75rem 0", fontFamily: "var(--font-dm-mono),monospace",
    fontSize: "0.82rem", color: "#f0ede6", outline: "none", transition: "border-color 0.3s",
  });

  return (
    <section id="contact" style={{ position: "relative", padding: "8rem 0", background: "#0d0d0d", overflow: "hidden" }}>
      <div style={{ position: "absolute", top: "3rem", right: "2rem", fontFamily: "var(--font-syne),sans-serif", fontWeight: 800, fontSize: "clamp(4rem,12vw,8rem)", color: "#1a1a1a", lineHeight: 1, pointerEvents: "none", userSelect: "none" }}>04</div>
      ...
      <div className="section-inner">
        ...
        <div className="two-col">
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: "1.5rem" }}>
            <h2 style={{ fontFamily: "var(--font-syne),sans-serif", fontWeight: 800, fontSize: "clamp(1.8rem,4vw,3.5rem)", color: "#f0ede6", lineHeight: 1.2 }}>
              Let&apos;s build something <span style={{ color: "#caff33" }}>great together</span>
            </h2>
            <p style={{ fontFamily: "var(--font-dm-mono),monospace", fontSize: "0.82rem", color: "#888", lineHeight: 1.85, maxWidth: "400px" }}>
              Have a project in mind? Looking for a developer? Or just want to say hello? My inbox is always open.
            </p>
            <a href="mailto:wajahatmustafa1234@gmail.com"
              style={{ display: "inline-flex", alignItems: "center", gap: "1rem", fontFamily: "var(--font-dm-mono),monospace", fontSize: "clamp(0.7rem,1.5vw,0.82rem)", color: "#f0ede6", textDecoration: "none", transition: "color 0.3s", wordBreak: "break-all" }}
            ...
            <div>
              <div style={{ fontFamily: "var(--font-dm-mono),monospace", fontSize: "0.6rem", textTransform: "uppercase", letterSpacing: "0.15em", color: "#555", marginBottom: "1rem" }}>Find me on</div>
              {socials.map(s => (
          ...
          <div>
            {status === "sent" ? (
              <div style={{ textAlign: "center", padding: "3rem 2rem", border: "1px solid rgba(202,255,51,0.3)", background: "rgba(202,255,51,0.04)" }}>
                ...
                <p style={{ fontFamily: "var(--font-dm-mono),monospace", fontSize: "0.78rem", color: "#888", marginBottom: "1.5rem" }}>I'll get back to you within 24 hours.</p>
              ...
              <form ref={formRef} onSubmit={submit} style={{ display: "flex", flexDirection: "column", gap: "2rem" }}>
                {[
                  { key: "name",  label: "Your Name *",     type: "text",  name: "from_name",  placeholder: "John Doe"            },
                  { key: "email", label: "Email Address *", type: "email", name: "from_email", placeholder: "john@example.com"    },
                ].map(f => (
                  <div key={f.key}>
                    <label style={{ fontFamily: "var(--font-dm-mono),monospace", fontSize: "0.6rem", textTransform: "uppercase", letterSpacing: "0.15em", color: "#555", display: "block", marginBottom: "0.5rem" }}>{f.label}</label>
                ...
                <div>
                  <label style={{ fontFamily: "var(--font-dm-mono),monospace", fontSize: "0.6rem", textTransform: "uppercase", letterSpacing: "0.15em", color: "#555", display: "block", marginBottom: "0.5rem" }}>Message *</label>
                ...
                <button type="submit" disabled={status === "sending"}
                  style={{ width: "100%", background: "#caff33", color: "#0a0a0a", border: "none", fontFamily: "var(--font-dm-mono),monospace", fontSize: "0.75rem", textTransform: "uppercase", letterSpacing: "0.1em", padding: "1rem", cursor: status === "sending" ? "not-allowed" : "none", fontWeight: 500, transition: "all 0.3s", opacity: status === "sending" ? 0.6 : 1, display: "flex", alignItems: "center", justifyContent: "center", gap: "0.5rem" }}
                ...
                >
                  {status === "sending" ? (
                    <><span style={{ width: "14px", height: "14px", border: "2px solid #0a0a0a", borderTopColor: "transparent", borderRadius: "50%", display: "inline-block", animation: "spin 0.7s linear infinite" }} />
                    <style>{`@keyframes spin{to{transform:rotate(360deg)}}`}</style>Sending...</>
                  ) : "Send Message →"}
                </button>
                <p style={{ fontFamily: "var(--font-dm-mono),monospace", fontSize: "0.65rem", color: "#555", textAlign: "center" }}>* I typically respond within 24 hours</p>
              </form>
            )}
          </div>
        </div>
